...
function drawPencil(cnv, start_x, start_y, end_x, end_y)
  local image = cnv.image
  local Pen_canvas = cd.CreateCanvas(cd.IMIMAGE,image)
  Pen_canvas:Foreground(cd.EncodeColor(0, 0, 255))
  Pen_canvas:Line(start_x, start_y, end_x, end_y)
  Pen_canvas:Kill()
  canvas.start_x = end_x
  canvas.start_y = end_y
end
...
function canvas:motion_cb(x, y, status)
  ...
  if (image) then
    ...
    if (iup.isbutton1(status)) then -- button1 is pressed 
      ...
      if self.shape == "PENCIL" then
        drawPencil(self, canvas.start_x, canvas.start_y, x, y)
      end
...
hbox = iup.hbox{
  iup.vbox {
    iup.button{ title="Pencil", tip="Pencil", action = Pencil_button},
